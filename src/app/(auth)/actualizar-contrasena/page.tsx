...
import { ArrowRight, Loader2, Lock, AlertCircle, CheckCircle2 } from "lucide-react";
  ...
  return (
    <main className="min-h-screen bg-[#0B0F19] relative flex flex-col items-center justify-center p-4 sm:p-6 overflow-hidden">
      <div className="absolute inset-0 pointer-events-none z-0">
        ...
        <div className="absolute -bottom-1/4 -left-1/4 w-[100wv] h-[100wv] bg-green-700/5 rounded-full blur-[120px]" />
      </div>
      ...
      >
        <div className="flex flex-col items-center mb-8 relative z-10 pt-20 md:pt-28">
          <h1 className="font-display text-4xl text-white tracking-wide text-center">
            Actualizar <span className="italic text-gold-500">Contraseña</span>
          </h1>
          <p className="text-slate-400 text-sm mt-3 font-light text-center">
            Ingresá tu nueva contraseña segura para recuperar tu cuenta.
          </p>
        </div>
          ...
          <AnimatePresence mode="wait">
            {serverState?.success ? (
              ...
              >
                ...
                <h3 className="font-display text-2xl text-white mb-3">¡Contraseña Guardada!</h3>
                <p className="text-slate-300 text-sm leading-relaxed mb-6">
                  {serverState.message}
                </p>
                <Link 
                  href="/club"
                  className="w-full glass-panel-glow bg-gold-500 text-slate-900 py-3 rounded-xl font-bold flex items-center justify-center hover:bg-gold-400 transition-all"
                >
                  Ir a mi Club
                </Link>
              </motion.div>
              ...
              >
                ...
                >
                  {isPending ? (
                    <>Actualizando <Loader2 size={18} className="animate-spin" /></>
                  ) : (
                    <>Guardar Contraseña <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" /></>
                  )}
                </button>
              </motion.form>
            )}
